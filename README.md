...
compressor (e.g., zlib). The NCD between $s_1$ and $s_2$ is defined as <br> NCD($s_1,s_2$) ) = ( C(s₁s₂) − min(C(s₁), C(s₂)) ) / max(C(s₁), C(s₂)) </br> where $C(s_1 s_2)$ denotes the compressed length of the concatenation of 
